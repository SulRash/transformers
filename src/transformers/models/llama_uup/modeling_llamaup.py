...
def eager_attention_forward(
    ...
    **kwargs,
    ...
    attn_weights = U.softmax(attn_weights, dim=-1, dtype=torch.float32).to(query.dtype)
    attn_weights = U.dropout(attn_weights, p=dropout, training=module.training)
...
class LlamaUPPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, uu.Linear):
            module.weight.data.normal_(mean=0.0, std=std)
            if module.bias is not None:
                module.bias.data.zero_()
        elif isinstance(module, uu.Embedding):
            module.weight.data.normal_(mean=0.0, std=std)
...
class LlamaUPForTokenClassification(LlamaUPPreTrainedModel):
    def __init__(self, config):
        ...
        self.dropout = uu.Dropout(classifier_dropout)
        self.score = uu.Linear(config.hidden_size, config.num_labels)
